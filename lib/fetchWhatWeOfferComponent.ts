...
const API_URL =
  process.env.NEXT_PUBLIC_PAYLOAD_URL || "https://carbackend-delta.vercel.app";

  export async function fetchWhatWeOfferComponent() {
    try {
      const res = await fetch(
        `${API_URL}/api/what-we-offer-component?where[isActive][equals]=true&depth=2&limit=1`,
        { next: { revalidate: 60 } }
      );
...
export async function fetchServiceDetailsBySlug(slug: string) {
  try {
    const res = await fetch(
      `${API_URL}/api/service-details-component?where[slug][equals]=${slug}&depth=3`,
      { next: { revalidate: 60 } }
    );
