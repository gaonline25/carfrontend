...
export async function fetchAboutHeroComponentData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/about-hero-component?where[isActive][equals]=true&depth=3&limit=1`, // Increased depth for safety
      {
        next: { revalidate: 0 }, // Disable revalidation for fresh data
