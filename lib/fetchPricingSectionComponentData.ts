...
export async function fetchPricingSectionComponentData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/pricing-section-component?where[isActive][equals]=true&depth=2&limit=1`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok)
      throw new Error(
        `Failed to fetch Pricing Section data: ${res.statusText}`
      );
