...
export async function fetchCubeGalleryComponentData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/cube-gallery-component?where[isActive][equals]=true&depth=2&limit=1`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok)
      throw new Error(`Failed to fetch Cube Gallery data: ${res.statusText}`);
