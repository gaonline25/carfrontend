const PAYLOAD_API_URL =
  process.env.NEXT_PUBLIC_PAYLOAD_API_URL || "https://carbackend-delta.vercel.app";

// Fetch Carsy Page Component Data
export async function fetchGalleryPageComponentData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/api/gallery-page-component?where[isActive][equals]=true&depth=3&limit=1`,
      { next: { revalidate: 60 } }
    );
    ...
    console.error("Error fetching GalleryPage:", err);
    return null;
  }
}
