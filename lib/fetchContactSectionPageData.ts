...
export async function fetchContactSectionPageData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/contact-section-component?where[isActive][equals]=true&depth=2&limit=100&draft=false&trash=false`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to fetch Contact Section page data: ${res.statusText}`
      );
    }
