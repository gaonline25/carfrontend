...
// components/CubeGallery.tsx or wherever you use it

import { fetchCubeGalleryComponentData, getImageUrl } from "@/lib/fetchCubeGalleryComponentData";

type CubeGalleryData = Awaited<ReturnType<typeof fetchCubeGalleryComponentData>>;

export default async function CubeGallery() {
  const data: CubeGalleryData = await fetchCubeGalleryComponentData();

  if (!data) {
    return <div>Cube Gallery component not found or inactive.</div>;
  }
  ...
  return (
    <div
      data-w-id={mainSection.animationId || "d4f7ae61-ea69-bfe6-eb6b-3395c014e551"}
      style={{ opacity: 0 }}
      className={mainSection.className || "relative-page-wrapper"}
    >
      <div className={wrapperSettings.mainWrapperClassName || "main-wrapper"}>
        <div className="cube-section">
          {/* Left Text Content */}
          {leftTextContent.show && (
            <div
              id={leftTextContent.id || "w-node-c54a0890-70d9-86e9-634d-1428583b96a4-ab3f40e0"}
              className={leftTextContent.className || "text-content left"}
            >
              <div
                data-w-id={leftTextContent.innerContent?.animationId || "b8fead14-6ce2-753b-30a0-90191b5c6310"}
                style={{ opacity: 0 }}
                className={leftTextContent.innerContent?.className || "cube-text-con left"}
              >
                <h3 className={leftTextContent.innerContent?.titleClassName || "feature-title"}>
                  {leftTextContent.innerContent?.title}
                </h3>
                <p className={leftTextContent.innerContent?.descriptionClassName || "feature-description"}>
                  {leftTextContent.innerContent?.description}
                </p>

                {leftTextContent.ctaButton?.show && (
                  <div className={leftTextContent.ctaButton.wrapperClassName || "btn-wrapper"}>
                    <a
                      href={leftTextContent.ctaButton.url}
                      target={leftTextContent.ctaButton.openInNewTab ? "_blank" : undefined}
                      rel={leftTextContent.ctaButton.openInNewTab ? "noopener noreferrer" : undefined}
                      className={leftTextContent.ctaButton.className || "primary-white-button w-button"}
                    >
                      {leftTextContent.ctaButton.text}
                    </a>
                  </div>
                )}
              </div>
          ...
          {/* Cube Wrapper */}
          <div
            id={wrapperSettings.cubeWrapperContainerId || "w-node-c54a0890-70d9-86e9-634d-1428583b96a9-ab3f40e0"}
            data-w-id={wrapperSettings.cubeWrapperDataWId || "c54a0890-70d9-86e9-634d-1428583b96a9"}
            className={wrapperSettings.cubeWrapperClassName || "cube-wrapper"}
          >
            {cubeFaces.map((face: any) => (
              <div key={face.id} className={face.className}>
                {/* Face Image */}
                <img
                  sizes={face.image.sizes || "(max-width: 479px) 60vw, (max-width: 767px) 80vw, (max-width: 991px) 8vw, 30vw"}
                  srcSet={face.image.srcSet || undefined}
                  src={getImageUrl(face.image.upload) || face.image.src || ""}
                  alt={face.image.alt || "Project photo"}
                  loading={face.image.loadingStrategy || "lazy"}
                  className={face.image.className || "full-image"}
                />

                {/* Overlay */}
                {face.overlay?.show && (
                  <div className={face.overlay.className || "overlay-gradient"}></div>
                )}

                {/* Lightbox */}
                {face.lightbox?.enable && (
                  <a href="#" className={face.lightbox.className || "cube-lightbox w-inline-block w-lightbox"}>
                    <div className={face.lightbox.circleClassName || "lightbox-circle"}>
                      <p className={face.lightbox.iconClassName || "lightbox-icon"}></p>
                    </div>

                    {/* Lightbox JSON Script (critical for Webflow lightbox) */}
                    <script
                      type="application/json"
                      className="w-json"
                      dangerouslySetInnerHTML={{
                        __html: JSON.stringify({
                          items: face.lightbox.images?.map((img: any) => ({
                            _id: img.id || img._id,
                            url: getImageUrl(img.upload) || img.url || "",
                            type: img.type || "image",
                            width: img.width,
                            height: img.height,
                            fileName: img.fileName || img.origFileName,
                            origFileName: img.origFileName || img.fileName,
                            fileSize: img.fileSize,
                          })) || [],
                          group: face.lightbox.galleryGroup || "Cube Gallery",
                        }),
                      }}
                    />
                  </a>
          ...
          {/* Right Text Content */}
          {rightTextContent.show && (
            <div
              id={rightTextContent.id || "w-node-c54a0890-70d9-86e9-634d-1428583b96b2-ab3f40e0"}
              className={rightTextContent.className || "text-content right"}
            >
              <div
                data-w-id={rightTextContent.innerContent?.animationId || "c621d4ce-8f5e-ca2f-28b9-1eb32f30d128"}
                style={{ opacity: 0 }}
                className={rightTextContent.innerContent?.className || "cube-text-con"}
              >
                <h3 className={rightTextContent.innerContent?.titleClassName || "feature-title"}>
                  {rightTextContent.innerContent?.title}
                </h3>
                <p className={rightTextContent.innerContent?.descriptionClassName || "feature-description"}>
                  {rightTextContent.innerContent?.description}
                </p>

                {rightTextContent.ctaButton?.show && (
                  <div className={rightTextContent.ctaButton.wrapperClassName || "btn-wrapper"}>
                    <a
                      href={rightTextContent.ctaButton.url}
                      target={rightTextContent.ctaButton.openInNewTab ? "_blank" : undefined}
                      rel={rightTextContent.ctaButton.openInNewTab ? "noopener noreferrer" : undefined}
                      className={rightTextContent.ctaButton.className || "primary-button w-button"}
                    >
                      {rightTextContent.ctaButton.text}
                    </a>
                  </div>
                )}
              </div>
